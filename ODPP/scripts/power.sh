...
for bench in resnet20_v2 resnet32 resnet32_v2 resnet44 resnet44_v2 resnet56 resnet56_v2 resnet110 resnet110_v2 trivial densenet40_k12 densenet100_k12 densenet100_k24 nasnet
do    
    for bsize in 32 64 128 256
    do
        nvidia-smi -i 0 --loop-ms=100 --format=csv --query-gpu=power.draw > ${results_dir}/${data}/${bench}.${bsize}.pwr.txt &
        power_sid=$!

        python tf_cnn_benchmarks.py --num_gpus=1 --batch_size=${bsize} --model=${bench} --data_dir=${cifar_dir} --data_name=cifar10 --num_epochs=5 > ${results_dir}/${data}/${bench}.${bsize}.out.txt
